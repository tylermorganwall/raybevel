...
`rayskeleton` is an R package that can create and visualize straight
...
#> The following object is masked from 'package:rayskeleton':
#> 
#>     run_documentation
library(rayvertex)
#> 
...
Multiple bevels are also supported (here, we use raw offsets and
heights):

``` r
roof_bevel_multi = generate_beveled_polygon(skeleton, 
                                            bevel_offsets = c(0.25,0.5,1,1.5,2),
                                            bevel_heights = c(0.25,0.25,0.5,0.5,0.75)*2,
...
roof_bevel_multi = generate_beveled_polygon(skeleton, base_height = -0.5,
                                            ...
                                            bevel_heights = sqrt(seq(0,2.5,by=0.02))*2,
                                            raw_offsets = TRUE,
                                            raw_heights = TRUE) 
...
roof_bevel_multi_sin = generate_beveled_polygon(skeleton, 
                                                bevel_offsets = seq(0,2.5,by=0.02),
                                                bevel_heights = abs(sinpi(seq(0,2.5,by=0.02))),
                                                base = TRUE, base_height = -0.5,
                                                raw_offsets = TRUE, 
                                                raw_heights = TRUE) 
...
cali_skeleton = skeletonize(california)

roof_cali = generate_roof(cali_skeleton, base = TRUE, sides = TRUE, offset = 2, base_height = 0) |> 
  center_mesh() |> 
  scale_unit_mesh() |> 
  translate_mesh(c(0,0.5,0)) |>
  scale_mesh(c(10,2,10))
...
generate_ground(depth = -0.1,material = diffuse(color="grey50",checkercolor = "white",checkerperiod = 0.5)) |>
  ...
  add_object(sphere(y=10,x=20,z=-50,radius=10,material=light(color="white", intensity = 20))) |>
  add_object(sphere(y=20,x=30,z=50,radius=10,material=light(color="white", intensity = 20))) |>
  add_object(sphere(y=10,x=-20,z=-50,radius=10,material=light(color="lightblue", intensity = 20))) |>
  render_scene(lookfrom=c(0,10,9), camera_up=c(0,1,0), lookat=c(0,0,-0.5),
               sample_method = "sobol_blue", fov=0, ortho_dimensions = c(11,11),
...
``` r
ca_model = generate_beveled_polygon(cali_skeleton, offset=1, sides = TRUE,  double_sided = TRUE,
                                    bevel_offsets = bevel, base = TRUE, base_height = 0, swap_yz = FALSE,
