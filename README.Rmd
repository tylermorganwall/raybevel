...
<!-- badges: end -->
  ...
  `rayskeleton` is an R package that can create and visualize straight skeletons, generate offset polygons, and generate 3D roof models and 3D beveled polygons. The 3D beveled polygons can be given any arbitrary 1D bevel profile controlled by the user.
...
```{r simple_poly, fig.width=6,fig.height=6}
library(rayskeleton)
...
```{r plot_skeleton, fig.width=6,fig.height=6}
...
```{r offsets, fig.width=6,fig.height=6}
...
```{r multioffsets, fig.width=6,fig.height=6}
...
Let's design a simple roof and render it with rayrender. When base_height is less than the roof offset (default of 0), the sides of the polygon will also be extruded.
...
Multiple bevels are also supported (here, we use raw offsets and heights):
...
```{r complex_bevel, fig.width=6, fig.height=6} 
