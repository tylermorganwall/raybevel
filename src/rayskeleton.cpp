...
List skeletonize_rcpp(NumericMatrix vertices, List holes, double tolerance) {
  if(vertices(0,0) !=  vertices(vertices.nrow()-1,0) ||
    ...
    // Rcpp::Rcout << vertices(0,0) << " " << vertices(vertices.nrow()-1,0) << "\n";
  ...
  contours.push_back(NormalizeContour(vertices, 0));
  for(size_t i = 0; i < holes.size(); i++) {
    contours.push_back(NormalizeContour(Rcpp::as<NumericMatrix>(holes(i)), 0));
  }
