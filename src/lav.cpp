...
LAV::LAV(std::vector<point2f>& polygon,
    Float tol,
    SLAV* slav,
    int& current_vertex_id) : slav(slav), tol(tol) {

  size_t full_length = polygon.size();
  std::shared_ptr<LAVertex> temp_head =
    std::make_shared<LAVertex>(polygon[full_length-1],
                               LineSegment(polygon[full_length-2],polygon[full_length-1],current_vertex_id),
                               LineSegment(polygon[full_length-1],polygon[1],current_vertex_id),
                               this,
                               current_vertex_id,
                               &slav->debugfile,
                               tol);
  // std::shared_ptr<LAVertex> temp_head =
  //   std::make_shared<LAVertex>(polygon[0],
...
LAVertex* LAV::unify(LAVertex* vertex_a, LAVertex* vertex_b, point2f point) {
  ...
  // Rcpp::Rcout << normed_a_bisector << " " << normed_b_bisector << "\n";
...
LAVertex* LAV::unify_multi(LAVertex* vertex_a, LAVertex* vertex_b, point2f point) {
  //Generate a new LAVertex from input point2f and resolves adjacency to old edges.
  //Returns:
  //LAVertex of intersection point.
