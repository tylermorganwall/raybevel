...
SLAV::SLAV(std::vector<std::vector<point2f> > contours, Float _tol) {
  tol = _tol;
  current_vertex_id = 0;
  current_edge_id   = 0;
  // debugfile.open("debug_cpp.txt", std::ios::out);
...
LAV* SLAV::operator[](size_t i) {
  return(lavs[i].get());
}
int SLAV::TotalValidVertices() {
  int n_valid = 0;
  for(size_t i = 0; i < lavs.size(); i++) {
    LAVertex* tmp = lavs[i]->head;
    for(size_t j = 0; j < lavs[i]->len; j++) {
...
void SLAV::writeDebugLine(std::string msg) {
  // debugfile << "DEBUG:__name__:" << msg << "\n";
}
void SLAV::writeInfoLine(std::string msg) {
...
std::pair<Subtree, std::vector<Event>> SLAV::handle_split_event(Event event) {
  ...
  // v1->bisector = CorrectBisectorDirection(v1->bisector, v1->next, v1->prev,
  //                                         v1->edge_right, v1->edge_left,
  //                                         tol);
  // v2->bisector = CorrectBisectorDirection(v2->bisector, v2->next, v2->prev,
  //                                         v2->edge_right, v2->edge_left,
  //                                         tol);
...
Float Angle(point2f p0, point2f p1) {
  Float dx = p1.x() - p0.x();
  Float dy = p1.y() - p0.y();
  return atan2(dy, dx);
}

bool CompareSplitEvents(Event const& e1, Event const& e2) {
  Float angle1 = Angle(e1.intersection, e1.vertex_a->edge_left.origin);
  Float angle2 = Angle(e1.intersection, e2.vertex_a->edge_left.origin);
...
std::vector<std::pair<Subtree, std::vector<Event>>> SLAV::handle_multi_event(std::vector<Event> events) {
  ...
  //Loop through split
  std::sort(splitEvents.begin(),splitEvents.end(), CompareSplitEvents);
  while(!splitEvents.empty()) {
    if(splitEvents[0].IsObsolete()) {
      // Rcpp::Rcout << "Erasing Split event, size: " << splitEvents.size() << "\n";

      splitEvents.erase(splitEvents.begin());
      continue;
    }
    // Rcpp::Rcout << "Multi Split events, size: " << splitEvents.size() << "\n";
  ...
  while(!edgeEvents.empty()) {
    // Rcpp::Rcout << "Multi Edge events, size: " << edgeEvents.size() << "\n";
